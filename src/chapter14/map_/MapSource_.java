...
public class MapSource_ {
    public static void main(String[] args) {
        ...
        //   这时因为 static class Node<K,V> implements Map.Entry<K,V>    （接口的多态）
        //4. 当把 HashMap$Node 对象 存放到 entrySet 就方便我们的遍历, 因为 Map.Entry 提供了重要方法
        //   K getKey(); V getValue();

        // final class EntrySet extends AbstractSet<Map.Entry<K,V>> {
        Set set = map.entrySet();  //EntrySet<Entry<K,V>> ，即： transient Set<Map.Entry<K,V>> entrySet;
        System.out.println(set.getClass());// HashMap$EntrySet ，EntrySet是HashMap的一个内部类
        for (Object obj : set) {
